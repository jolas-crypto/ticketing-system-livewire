                ...
                <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                    <table class="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach ($supportLists as $support)
                                <tr wire:key="{{ $support->id }}">
                                    <td class="table-tbody">{{ $support->code }}</td>
                                    <td class="table-tbody">{{ $support->name }}</td>
                                    <td class="table-tbody">{{ $support->description }}</td>
                                    <td class="table-tbody"><span class="table-tbody-span">{{ ($support->status) ? 'Active' : 'In-Active' }}</span></td>
                                    <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-sm font-medium sm:pr-0">
                                        <a href="{{ route('show-support', $support->id) }}" wire:navigate class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
